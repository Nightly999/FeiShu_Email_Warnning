...
async def send_daily_welcome_once(app: TenantApp, event: dict[str, Any]) -> bool:
    ...
    if not settings.email_feature_enabled:
        return False
    try:
        if await get_email_account(event["tenant_key"], event["app_id"], open_id):
            return False
    except Exception:
        logger.exception("Failed to check email binding on chat entry: bot_code=%s open_id=%s", app.bot_code, open_id)
        return False

    delivery_date = datetime.now(WELCOME_TIMEZONE).date().isoformat() + ":email_login"
    claim = (
        event["tenant_key"],
        event["app_id"],
        event.get("bot_code"),
    ...
    try:
        card = (await _binding_result(event, replacing=False)).card
